...
Das von uns entwickelte System zur Kennzeichenerkennung stellt eine funktionale Lösung dar, die unter Berücksichtigung der begrenzten Hardware-Ressourcen des Raspberry Pi gute Ergebnisse erzielt. Hervorzuheben ist der praxisnahe Aufbau, der die zentralen Schritte Bildaufanahme, Objekterkennung (mittels YOLOv5) und Texterkennung (mit Tesseract) kombiniert. Verglichen mit professionellen Kennzeichenerkennungssystemen zeigt sich jedoch ein deutlicher Unterschied in Robustheit, Flexibilität und Genauigkeit. Industrielle Lösungen verfügen in der Regel über spezialisierte OCR-Modelle, umfangreiche Fehlerkorrekturmechanismen, integrierte Plausibilitätsprüfungen und leistungsstärkere Hardware. Diese Modelle sind in der Lage nahezu unter allen Umweltbedingungen zuverlässig zu arbeiten. Unser Modell hingegen stößt, im OCR-Bereich, bei stark wechselnden Lichtverhältnissen oder ungünstigen Kamerawinkeln an seine Grenzen. Auch die Echtzeitverarbeitung bleibt eingeschränkt und ist stark abhängig von der Systemauslastung.
Trotzdem zeigt das Projekt klar auf, wie sich moderne Objekterkennungstechnologie auch auf einfacher Hardware realisieren lässt. Die OCR stellte sich in unserem Projekt als einer der kritischsten Punkte heraus. Während die Kennzeichenerkennung mit YOLOv5 zuverlässig funktionierte, kam es bei der Zeichenerkennung durch Tesseract unter schwierigen Bedingungen häufiger zu Fehlinterpretationen. Gerade bei verzerrten Perspektiven, schwachem Kontrast oder leicht verschmutzten Kennzeichen stößt die Standard-OCR schnell an ihre Grenzen. Professionelle Systeme arbeiten hier oft mit speziell trainierten Netzen die auf Kfz-Schriftarten und -Layouts optimiert sind.\singlespacing 
Ein praktisches Beispiel dafür zeigt die Ausgabe unseres Systems.
Obwohl mehrere Aufnahmen desselben Kennzeichens gemacht wurden, variiert das erkannte Ergebnis leicht. So wird z.B. aus „RAKL8136“ mehrfach „ARAKL813“ oder „RAKL84136“.Auch wenn die erkannten Texte teils hohe Konfidenzwerte zwischen 87 und 95 Prozent aufweisen, verdeutlicht dies die Grenzen der eingesetzten OCR unter realen Bedingungen.\singlespacing
Trotzdem zeigt das Projekt klar auf, wie sich moderne Objekterkennungstechnologie auch auf einfacher Hardware realisieren lässt. Die OCR stellte sich in unserem Projekt als einer der kritischsten Punkte heraus. Während die Kennzeichenerkennung mit YOLOv5 zuverlässig funktionierte, kam es bei der Zeichenerkennung durch Tesseract unter schwierigen Bedingungen häufiger zu Fehlinterpretationen. Gerade bei verzerrten Perspektiven, schwachem Kontrast oder leicht verschmutzten Kennzeichen stößt die Standard-OCR schnell an ihre Grenzen. Professionelle Systeme arbeiten hier oft mit speziell trainierten Netzen die auf Kfz-Schriftarten und -Layouts optimiert sind.\singlespacing Abschließend lässt sich sagen, dass unser System in seiner aktuellen Form eine solide funktionale Basis für einfache Szenarien bietet, jedoch im Vergleich zu professionellen Lösungen noch deutliche Einschränkungen aufweist.

\begin{figure}[h]
    \centering
<<<<<<< HEAD
    \includegraphics[width=0.55\textwidth]{data/Genauigkeit.png}
=======
    \includegraphics[width=0.5\textwidth]{data/Genauigkeit.png}
>>>>>>> 48cd96aee9963a7958702987442d3de1d39bb0ef
    \caption{Terminal Ausgabe der Genauigkeit}
...
\subparagraph{}{Confidence Threshold und FPS-Erklärung}

Für die Bewertung der Gesichtserkennung mit \texttt{YOLOv8n-face} wurde die Verteilung der \texttt{detection\_scores} analysiert. Diese liegt im Mittel bei etwa $\mu = 0{,}937$ mit einer relativ geringen Varianz. Die Konfidenzwerte bleiben in der Regel über $0{,}85$, mit vereinzelten Ausreißern darunter. Auf Basis dieser Analyse wurde ein \textbf{Confidence Threshold von 0{,}90} gewählt, um eine gute Balance zwischen Erkennungsrate und Präzision zu erreichen. Werte unterhalb dieser Schwelle führten häufiger zu fehlerhaften oder instabilen Erkennungen.

Frames per Second (FPS). Die \textbf{FPS} gibt an, wie viele Bilder pro Sekunde das System verarbeiten kann. In unserem Fall dient dieser Wert als Kennzahl für die Echtzeittauglichkeit des Systems. Je höher der FPS-Wert, desto flüssiger läuft die Gesichtserkennung bei Videoeingaben. Bei zu niedrigen FPS kann die Erkennung verzögert oder ruckelig erscheinen, was insbesondere bei Überwachungs- oder Interaktionssystemen problematisch wäre. Ein Wert von \textbf{15–30 FPS} gilt als untere Grenze für akzeptable Echtzeitleistung, wobei unser System je nach Konfiguration und Optimierung diese Marke im Mittel erreicht oder übertrifft (siehe Abb. \ref{fig:fps-diagramm}).

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.55\textwidth]{data/fps_yolo_detection.png}
    \caption{Verlauf der FPS während der Gesichtserkennung}
    \label{fig:fps-diagramm}
\end{figure}
...
\begin{itemize}
    \item Die durchschnittliche Bildrate (FPS) steigt bei einem zunehmenden Confidence-Wert von 50\% auf 80\% von 16,8 FPS auf 20,6 FPS.
    ...
    \item Die Stabilität der Erkennung steigt leicht an, von 97,7\% bei 50\% auf 98,3\% bei 80\% Confidence.
\end{itemize}
...
Die \texttt{detection\_scores} von \texttt{YOLOv8n-face} lagen im Mittel bei $\mu = 0{,}937$ mit geringer Varianz. Meist über $0{,}85$, wurde ein \textbf{Threshold von 0{,}90} gewählt, da darunter vermehrt fehlerhafte Erkennungen auftraten.

\texttt{MediaPipe} lieferte in ruhigen Szenen meist Konfidenzen über $95\,\%$, bei schnellen Bewegungen jedoch deutlich niedrigere Werte. Während YOLO robuster gegenüber Bewegung ist, zeigt MediaPipe bei stabilen Bedingungen höhere Genauigkeit.
